...
class SRGAN_B(nn.Module):
    def __init__(self, image_channels: int, feature_maps: int = 64, num_res_blocks: int = 16,
                 num_ps_blocks: int = 2) -> None:
        super().__init__()
        # Input block (k9n64s1)
        self.input_block = nn.Sequential(
            nn.Conv2d(image_channels, feature_maps, kernel_size=9, padding=4),
        ...
        # k3n64s1
        res_blocks += [
            nn.Conv2d(feature_maps, feature_maps, kernel_size=3, padding=1),  # change transpose here
            nn.BatchNorm2d(feature_maps),
        ]
        self.res_blocks = nn.Sequential(*res_blocks)

        ps_blocks = []
        for i in range(num_ps_blocks):
            m = np.int(2 ** i)
            ps_blocks += [
                nn.Conv2d(feature_maps * m, int(feature_maps / 2), kernel_size=3, padding=1),
                nn.PixelUnshuffle(2),
                nn.PReLU(),
        ...
        self.output_block = nn.Sequential(
            nn.Conv2d(2 * feature_maps, image_channels, kernel_size=9, padding=4),
            nn.Tanh(),
        )
